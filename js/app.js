...
function renderNav(active) {
  ...
  el.innerHTML = '<div class="nav-inner">'
    ...
    + links.map(function (l) {
        const isJoin = l[0] === "membership.html";
        return '<a class="navlink' + (l[0] === active ? ' active' : '') + (isJoin ? ' nav-join' : '') + '" href="' + l[0] + '">' + l[1] + '</a>';
      }).join("")
    + '</div>';
}
...
function renderFooter() {
  ...
  if (!el) return;
  el.innerHTML = "LyDia on mlbedges.com — analysis and education only, not betting advice. "
    + "Odds and stats can change quickly; always verify with your sportsbook. "
    + "No model or pick guarantees a profit. Please bet responsibly. If gambling stops being fun, call 1-800-GAMBLER.";
}
