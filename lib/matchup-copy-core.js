  ...
  if ((breakdown.pitching_plan_points / LAB_MAX.pitching_plan) < LAB_WEAK_FRACTION) {
    if (pitcherEdgeTeam && pitcherEdgeTeam === pickTeam && isNum(pitcherGap)) {
      reasons.push({
        ...
        detail: `${betterPitcher || pitcherEdgeTeam} rates ${pitcherGap} points better than ${worsePitcher || "the opposing starter"} on Leo's pitcher score — `
  ...
  if ((breakdown.bullpen_points / LAB_MAX.bullpen) < LAB_WEAK_FRACTION) {
    reasons.push({
      ...
      detail: bullpenPickLabel && bullpenOppLabel && bullpenPickLabel === bullpenOppLabel
        ...
        : `Leo does not see a meaningful bullpen edge for ${pickTeam} once assigned innings are weighted.`
    });
  }
...
function labRatingBreakdown({
  ...
  const planDetail = (pitcherEdgeTeam && pitcherEdgeTeam === pickTeam && isNum(pitcherGap))
    ? `${betterPitcher || pitcherEdgeTeam} rates ${pitcherGap} points better than ${worsePitcher || "the opposing starter"} on Leo's pitcher score.`
  ...
  const bpDetail = strong(bpFrac)
    ...
    : (bullpenPickLabel && bullpenOppLabel && bullpenPickLabel === bullpenOppLabel
        ? `Both bullpens are rated ${String(bullpenPickLabel).toLowerCase()} -- this is close to a wash, not an advantage either way.`
        : `Leo does not see a meaningful bullpen edge for ${pickTeam} here.`);
...
/*
  ...
  consistency question against Leo's own past analysis, without inventing
...
function coachBucketNote({ dimension, pickSide, oppositeSide, pickLabel, oppositeLabel }) {
  ...
  return {
    title: `Coach: ${dimension} is Leo's weaker bucket`,
    detail: `This pick falls in "${pickLabel}" (${pickSide.wins}-${pickSide.losses}, ${pct(pickSide.rate)}) in Leo's current-model sample, versus "${oppositeLabel}" (${oppositeSide.wins}-${oppositeSide.losses}, ${pct(oppositeSide.rate)}). A review prompt only, per Coach's own rule -- not a reason to skip or fade this pick.`
  };
}
...
function coachTierNote({ dimension, tiers, pickIndex }) {
  ...
  return {
    title: `Coach: ${dimension} is Leo's weaker bucket`,
    detail: `This pick falls in "${pick.label}" (${pick.wins}-${pick.losses}, ${pct(pick.rate)}) in Leo's current-model sample, versus "${best.label}" (${best.wins}-${best.losses}, ${pct(best.rate)}), its best-performing tier. A review prompt only, per Coach's own rule -- not a reason to skip or fade this pick.`
  };
}
